...
public class SeguimientoResource {
    ...
    public ResponseEntity<byte[]> generateFile(SeguimientoCriteria criteria, HttpServletResponse response)
        throws IOException, URISyntaxException {
        log.debug("REST request to get Seguimientos and Export by criteria: {}", criteria);
        List<SeguimientoDTO> seguimientos = seguimientoQueryService.findByCriteria(criteria);
        if (seguimientos.size() != 0) {
            File file = seguimientoService.generateFile(
                seguimientos,
                seguimientos.get(0).getObraName(),
                seguimientos.get(0).getPeriodName()
            );
