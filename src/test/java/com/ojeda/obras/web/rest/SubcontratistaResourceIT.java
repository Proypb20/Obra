...
class SubcontratistaResourceIT {
    ...
    @Mock
    private SubcontratistaRepository subcontratistaRepositoryMock;

    @Autowired
    private SubcontratistaMapper subcontratistaMapper;

    @Mock
    private SubcontratistaService subcontratistaServiceMock;
    ...
    void getAllSubcontratistasWithEagerRelationshipsIsEnabled() throws Exception {
        when(subcontratistaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSubcontratistaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(subcontratistaServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllSubcontratistasWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(subcontratistaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSubcontratistaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(subcontratistaRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
